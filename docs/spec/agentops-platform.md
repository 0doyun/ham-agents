...
| 실시간 차단 (block before execution) | hook 단방향 → 사전 차단 불가 | ✗ 알림만 가능 |
...
**결론**: Org Policy Engine 의 핵심 제약은 **hook 단방향으로 인해 차단(block)은 불가능하고 알림(notify)만 가능**하다는 점이다. "Policy Engine" 이라는 이름은 "정책으로 에이전트를 제어한다"는 기대를 심지만, 실제 동작은 "정책 위반을 감지하고 사용자에게 알린다" 수준이다. 이를 명확히 하기 위해 기능 설명에 **(Best-effort, hook 제약 있음 — 차단 불가, 알림 전용)** 태그를 부착한다. `~/.claude/settings.json`의 `permissions.deny` 사전 연동으로 특정 도구 원천 차단은 별도로 가능하다.
