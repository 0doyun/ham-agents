...
Phase 1에서는 IPC 프로토콜 변경 없음. 기존 51개 Command 상수와 Request/Response 구조체를 그대로 유지한다. EventBus는 서버 내부에서만 동작하며, 외부 클라이언트는 기존 폴링 방식으로 계속 접근 가능.
...
`SessionEvent` -- 모든 이벤트의 정규화된 공통 스키마:
...
type SessionEvent struct {
    ...
    AgentID   string    `json:"agent_id"`
    ParentID  string    `json:"parent_id,omitempty"`  // 부모 에이전트/이벤트
    TaskID    string    `json:"task_id,omitempty"`     // 태스크 그룹핑
    
    // Metadata
    Type       EventType `json:"type"`
    Source     string    `json:"source"`               // "hook", "poll", "pty", "log", "manual"
    OccurredAt time.Time `json:"occurred_at"`
    
    // Content
    Summary     string            `json:"summary"`
    Payload     map[string]any    `json:"payload,omitempty"`     // 타입별 구조화 데이터
    ArtifactRef string            `json:"artifact_ref,omitempty"` // 파일/diff/PR 참조
    
    // Confidence (debugger/policy engine 자산)
    Confidence      float64 `json:"confidence"`
    ConfidenceModel string  `json:"confidence_model,omitempty"` // "hook_direct", "pty_inferred", "log_parsed"
...
**Mode 통합**: Managed/Attached/Observed는 `Source` 필드로 구분한다.
...
| 파일 | 변경 내용 |
|------|-----------|
| `go/internal/core/agent.go` | `Event` 구조체에 `SessionID`, `ParentID`, `TaskID`, `Source`, `Confidence`, `ConfidenceModel`, `ArtifactRef`, `ApprovalState`, `Payload` 필드 추가 |
| `go/internal/core/agent.go` | `CostInfo` 구조체 신규 정의, `Event`에 `Cost *CostInfo` 필드 추가 |
...
- `dispatch()` (`server.go:136-634`): 거대한 switch 문 (51개 case)
...
| 기존 유지 | 51개 Command + Request/Response 구조체 그대로 |
...
| `go/internal/ipc/ipc.go` | IPC 프로토콜 | `Command` (51개), `Request`, `Response`, `Client` |
| `go/internal/ipc/server.go` | IPC 서버 | `Server`, `dispatch()` (51-case switch), `prepareHookRequest()` |
