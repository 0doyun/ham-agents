...
└── internal/
    ...
    ├── ipc/           # Unix 소켓 IPC (Command 51개, Server dispatch)
...
| 파일 | 역할 |
|------|------|
| `ipc.go` (605줄) | 전체 Command 상수 51개, Request/Response 스키마, Client 구현 |
...
| `DaemonIPC.swift` | DaemonCommand (16 케이스), DaemonRequest/Response, 설정 페이로드 |
...
**Swift DaemonCommand** (16개) vs **Go Command** (51개):
...
- 세션: `tmux.sessions`
- 데몬: `daemon.shutdown`
- Hook 전체: 31개 (Swift 프론트엔드에서 직접 사용하지 않음)
...
- [x] IPC 커맨드 전수 목록 (51개) + 각 커맨드의 request/response 스키마
