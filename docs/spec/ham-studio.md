...
개발자가 모노레포에서 Claude Code 세션 5개를 병렬로 돌리고 있다. 메뉴바 팝오버는 "5개 중 2개 대기 중" 요약만 보여준다. ham Studio 를 열면 좌측 트리에서 각 세션의 구체적 상태 (running_tool, waiting_input, error), 현재 실행 중인 tool 이름, 마지막 assistant 메시지를 한 화면에서 확인한다. error 상태 에이전트를 선택하면 중앙 패널에 이벤트 로그, 우측 패널에 에러 상세와 artifact diff 가 표시된다. "Go to Terminal" 버튼 하나로 실제 iTerm2/tmux 세션으로 점프해 개입할 수 있다. 탭을 전환하며 일일이 확인하는 대신, Studio 한 창에서 전체 조종이 완료된다.

### 시나리오 2: 승인 워크플로 (approval loop)

Claude Code 가 프로덕션 DB 마이그레이션 스크립트를 실행하려 한다. 사용자는 Studio 우측 패널의 Approval Inbox 에서 permission-request 내용 — 실행할 명령, 대상 파일, 요청 맥락 — 을 한눈에 확인한다. approve 를 클릭하면 hamd 가 해당 결정을 기록하고 Phase 2 에서는 Claude Code permission 시스템을 경유해 응답한다. deny 시 에이전트는 다른 경로를 탐색한다. 메뉴바만 쓸 때는 notification 을 받고 터미널로 가서 수동 입력해야 했지만, Studio 에서는 워크플로가 한 창 안에서 완결된다.

### 시나리오 3: 팀 오케스트레이션 관찰

lead 에이전트가 "API v2 마이그레이션" 작업을 3개 worker 에이전트에 분배했다. Studio 좌측 트리에서 팀 그룹과 각 worker 의 담당 branch, 변경 파일 수, 현재 상태를 시각적으로 확인한다. Worker B 가 conflict 로 error 상태가 되면 트리에서 즉시 감지한다. Playbook 을 통해 "conflict 발생 시 lead 에이전트에게 보고" 규칙을 미리 설정해두면 사용자가 지켜보지 않아도 팀 흐름이 자동 관리된다. 각 worker 의 worktree 격리 상태, 완료된 task 목록, 비용 소비량을 우측 Inspector 패널에서 팀 단위로 집계해서 본다.
...
Phase 1의 읽기 전용 Notification Inbox를 승인/거절 가능한 Approval Inbox로 업그레이드한다. **단, ADR-2 조사에서 Claude Code 외부 승인 API가 확인된 경우에만 구현.**
...
- **필수:** ADR-2 결론 — Claude Code 외부 승인 API 존재 확인
...
| 불가능 | 이유 | 대안 |
|---|---|---|
| Claude Code permission 외부 승인 (ADR-2 미정) | 2026.04 기준 공개 API 미확인 | **대안 A:** Claude Code MCP server로 ham이 approval tool을 제공 → Claude Code가 ham에 tool call로 승인 요청 → ham이 사용자에게 UI 표시 → 결과를 tool response로 반환. **대안 B:** iTerm2 AppleScript로 키 입력 시뮬레이션 (y/n 입력) — fragile하지만 동작 가능. **대안 C:** API 공개 대기 후 Phase 3에서 구현 |
